...
func (p *Planner) Plan(stmt *influxql.SelectStatement, chunkSize int) (Executor, error) {
	...
	if stmt.IsRawQuery && !stmt.HasDistinct() {
		return p.planRawQuery(stmt, shards, chunkSize)
	}
	return p.planAggregateQuery(stmt, shards)
}

func (p *Planner) planRawQuery(stmt *influxql.SelectStatement, shards map[uint64]meta.ShardInfo, chunkSize int) (Executor, error) {
	...
	re := NewRawExecutor(stmt, mappers)
	re.ChunkSize = chunkSize
	return re, nil
}
...
func (ae *AggregateExecutor) execute(out chan *influxql.Row) {
	...
	// a given interval. The data will be "in the right place" XXX FIX THIS COMMENT MOVE TO OPEN?
	...
	// Put together the rows to return, starting with columns. XXX MOVE TO OPEN?
	...
	for !ae.mappersDrained() {
		...
		// Prep a row, reading for kicking out.
		var row *influxql.Row

		// Prep for bucketing data. XXX Better comment, perhaps even new types.
